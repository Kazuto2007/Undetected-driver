...
namespace request
{
    ...
    template <class t> 
    t read_kernel_virtual( std::uintptr_t address ) {
    ...
    auto get_dtb( invoke_data* request ) -> driver::status
    {
        ...
        std::uintptr_t process_dtb = read_kernel_virtual<std::uintptr_t>( (std::uintptr_t)process + 0x28);
        if ( !process_dtb ) {
            process_dtb = read_kernel_virtual<std::uintptr_t>( ( std::uintptr_t ) process + 0x388);
        }
    ...
    auto read_memory( invoke_data* request ) -> driver::status
    {
        ...
        // sometimes does not work, fix it or kys? 
